...
export default function MetricGaugeChart({ metric, data, unit }: MetricGaugeChartProps) {
  ...
  const gaugeMax = maxVal <= 0 ? 100 : Math.ceil(maxVal * 1.2);
    ...
    series: [
      {
        ...
        data: [{ value: latestValue, name: metric.toUpperCase() }],
    ...
    <div ref={containerRef} className={`bg-white ${isFullscreen ? 'w-full h-[100dvh] p-6' : 'p-2 w-full h-[320px]'} flex flex-col overflow-hidden transition-all`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 border-b-2 border-slate-200 pb-2 gap-2">
        <div className="flex items-center gap-3">
          <h4 className="text-[#003366] font-bold text-lg tracking-wide uppercase">{metric}</h4>
          <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 font-bold uppercase tracking-widest border border-slate-300">Gauge</span>
        </div>
